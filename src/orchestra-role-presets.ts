...
function renderComposition(options: {
  ...
  const rows: string[] = [
    ...
    "  config:",
    "    prefix: " + JSON.stringify(rolePrompt(options.role, options.purpose, options.write, options.baseStrategy, options.reportHandoff)),
    "    complete: false",
...
export async function ensureBuiltinRolePresetArtifact(globalRoot: string, spec: RolePresetSpec): Promise<string> {
  ...
  } else if (existing.includes("name: '@deepseek-ai/dsh-persona'") && existing.includes("text:")) {
    const migrated = existing.replace(/([ \t]+)text:([ \t]*)/, "$1prefix:$2");
