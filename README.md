...
The driver runs the onboarding protocol (goal → constraints → context → **topology proposal** → **your approval** → execute), proposes a collaboration charter, and only after you approve creates the roles and dispatches self-contained tasks. There is no shortcut around that gate: `orchestra_create` requires a `frozenRef` produced by `orchestra_freeze`, so calling it with only a goal and a template returns `approval_required` by design.
...
- **Unreleased (2026-09-15)** — the decision records in `docs/adr/` now supersede older claims here. Landed since v0.4.1: plugin records moved out of Session logs into `<cwd>/orchestra/` files (a custom Session event type makes the whole log unreadable on reread — ADR-0002); approval by a plain one-word reply (ADR-0006); degraded session discovery now reports itself instead of silently returning a short list; the dev instance moved to port 4600 (ADR-0009).
