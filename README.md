...
**Orchestra lanes:** `orchestra_draft` (append-only plan draft; per-role blueprint preview rendered as a Markdown table) · **your approval** (a plain yes — 启动 / 可以 / ok — recorded against the exact draft revision last shown to you; only a genuine user turn counts, `/team approve <draftId>@<revision>` also works) · `orchestra_create` (provision the approved plan; requires the `frozenRef` that approval produced) · `orchestra_dispatch` (dispatch a task to a lane; creates and records the role session on demand) · `orchestra_wait` (sleep until a committed team write or the 20-minute heartbeat; reports booleans, never payloads) · `orchestra_report` (escrow write channel — the only durable write a read-only role has) · `orchestra_send` (dispatch to a governed role by teamId+roleId) · `orchestra_team` (objective board: role sessions, liveness, report counts, last report path) · `orchestra_dismiss` / `orchestra_activate` (archive & revive) · `orchestra_topologies` (project/global/built-in catalog).
