...
test("catalog resolves all bundled topologies and installs the same builtin truth", async () => {
  await withRoots(async ({ fs, projectRoot, globalRoot }) => {
    ...
    assert.deepEqual(list.ready.map((entry) => entry.config.id), ["architecture-decision", "audit-and-hardening", "bug-diagnosis-and-fix", "duo", "feature-development", "four-role-dev", "oracle", "refactor-and-migration", "trio"]);
    await catalog.ensureBundledArtifacts();
    assert.deepEqual((await readdir(join(globalRoot, "topologies"))).sort(), ["architecture-decision.json", "audit-and-hardening.json", "bug-diagnosis-and-fix.json", "duo.json", "feature-development.json", "four-role-dev.json", "oracle.json", "refactor-and-migration.json", "trio.json"]);
  });
});
