...
test("catalog resolves all bundled topologies and installs the same builtin truth", async () => {
  await withRoots(async ({ fs, projectRoot, globalRoot }) => {
    ...
    assert.deepEqual(list.ready.map((entry) => entry.config.id), ["duo", "feature-development", "four-role-dev", "oracle", "trio"]);
    await catalog.ensureBundledArtifacts();
    assert.deepEqual((await readdir(join(globalRoot, "topologies"))).sort(), ["duo.json", "feature-development.json", "four-role-dev.json", "oracle.json", "trio.json"]);
  });
});
