...
> ## ⚠️ 2026-09-12 方向修订（v0.5 目标，优先于本文件正文）
>
> 用户在本日口述了产品愿景，**它改变了 v0.4 的两条根本前提**，因此本文件正文中与之冲突的表述（§0.1 的 Session 定义、§1.1 的产品定位、§1.1 的「两种一等用法」表）**已降级为历史记录**。完整修订见 [附录 A](#附录-a-v05-方向修订2026-09-12)。摘要：
>
> 1. **角色 → 节点**：角色不再是「必须是完整 Session」，而是**节点**，可由两种后端承载 —— 可见 Session，或 DSH 原生 continuable 子 agent。分界不是偏好而是**原生强制**：需要独立 Agent Preset、需要向用户请求审批、需要独立 cwd 三者之一 → 必须用 Session。
> 2. **核心问题改为编排**：本插件要解决的是「**如何编排 workflow**」—— 有界无环图（DAG）+ 若干**有界 Loop**（受控回边）。这是产品中心，不是治理文档的附属。
> 3. **无人值守成为验收不变量**：编排必须能在无人盯着的情况下跑到明确终态。
> 4. **A2A 降级为手段**：A2A 是编排使用的传输机制，不再是与编排并列的第二条一等产品路径。
>
> 尚未在代码中落地的部分不得被表述为现状。
...
## 附录 A：v0.5 方向修订（2026-09-12）

> 来源：用户 2026-09-12 口述愿景。本附录**优先于正文**；与正文冲突处（§0.1 的 Session 定义、§1.1 的产品定位与「两种一等用法」表）以本附录为准，正文保留为历史记录。

### A.1 产品是什么：一套「多 agent 推进编排的设计方法」

**本插件里存的是比一张 DAG 更高阶的东西 —— 如何设计一套 multi-agent 的推进编排。** DAG 是这套方法的**产物**，不是本体。

`/team` 承载的是一条三步流水线：

| 步 | 谁做 | 产出 | 合同 |
|---|---|---|---|
| **1. 勾勒 mission** | 用户 + driver 对话 | objective / scope / constraints / acceptance criteria / non-goals | 必须与用户共同完成，不得由 driver 单方假定 |
| **2. 按原则成图** | driver agent 自己 | 一张 DAG：节点与边、若干 Loop、ownership、handoff 契约、Gate、Closure | 必须遵守 **A.3 的编排原则**；违反原则的图不得进入第 3 步 |
| **3. 解释 → 批准 → 派发** | driver 向用户解释，用户批准 | 自然语言可读的图解释 + 显式批准 | **未获批准不得冻结、不得创建任何节点**；批准是唯一硬门 |

因此产品的三样资产是：**原则**（怎么拆才科学）、**流程**（三步）、**运行时**（让图真的跑完）。**没有原则，DAG 只是一张图。**

「自然语言解释」是第 3 步的一等产物，不是附带动作：它必须足以让用户在不读 JSON 的前提下判断「这张图对不对」，并据此批准或驳回。

### A.2 图的模型：有界无环图 + 有界 Loop

- **DAG = 有向无环图**（Directed Acyclic Graph）。节点头部的「有界无边」按此解释。
- 叠加两条工程约束后才是本插件需要的图：
  1. **有界（bounded）**：节点集有限；**每条执行路径的长度有限**。
  2. **无环（acyclic）**：节点之间不得存在任意回边。
- **Loop 是 DAG 的必要补丁，但不破坏有界性**：Loop 引入回边，该回边**带次数上限**（`maxAttempts`）且有**明确出口**（pass / retry / capExhausted）。即 **Loop = 有界回边**，而非把图变成任意环路图。
- 一次编排中可以有**若干个 Loop**；每个 Loop 包含**若干节点**（典型 2–4 个）。

### A.3 编排原则（Multi-Agent Workflow Principles）

原则是第 2 步的判据。每条原则最终必须落在三处之一：**机器可校验的 validator 规则**、**driver 的可教文本（persona/skill）**、或**运行时强制**。仅写在文档里、无人执行的原则不算落地。

**设计类**

- **P1 单节点单职责**：一个节点只承担一个可判定的职责；职责无法用一句话说清 → 拆分。
- **P2 上下文独立优先于成本**：拆分的首要理由是**注意力可控与探究深度**（专项上下文），不是省 token。相邻职责若需要同一份深度上下文，应合并为一个节点而不是硬拆。
- **P3 节点后端按能力需求选择**（原生强制，非偏好）：需要**独立 Agent Preset**、需要**向用户请求审批**、或需要**独立 cwd** 三者之一 → 必须用**可见 Session**；否则用 **sub-agent**（join 父的 composition，可用 `persona`/`toolFilter`/`agentOptions` 钳制）。

  后端能力矩阵（2026-09-12 依据 DSH 0.1.5-rc.2 的真实类型契约核准，**不是偏好而是结构决定**）：

  | 能力 | `session` 节点 | `subagent` 节点 |
  |---|---|---|
  | 独立 Agent Preset（组合 / 工具面 / prompt 段） | ✅ | ❌ 只能 join 父的 live preset |
  | 独立 persona 段 | ✅（preset 内） | ✅ per-child shadow |
  | 独立工具面 | ✅ | ⚠️ 只能**收窄**（`toolFilter` allow/deny；未知工具名 fail loud） |
  | 独立 Permission Preset / sandbox | ✅ | ❌ 委派时冻结、继承父的显式 override |
  | **能向用户请求审批** | ✅ | ❌ 被钉死 `never`，asks 被确定性拒绝 |
  | 独立模型 / 思考强度 | ✅ | ✅ `agentOptions` |
  | 独立 cwd | ✅ | ❌ 共享父 cwd |
  | GUI 呈现 | 侧边栏平级行 | 挂在父下（后代计数 / lineage） |
  | 通用 Session 路由寻址 | ✅ | ❌ 必须走 parent 地址 |
  | 冷恢复 | ✅ | ✅（continuable descriptor） |
  | 相对成本 | 高（完整会话 + preset mount） | 低 |

  > 推论：**只读节点必须是 `session`**（子节点沙箱无法收窄到 read-only）；`toolFilter` 只是**可用性收窄，不构成权限保证**（bash 仍可写）。
- **P4 边即契约**：每条边必须声明 kind 与载荷契约；没有契约的边不得存在（避免"顺手传个话"式协作）。
- **P5 单一决策所有者**：每个决策恰好一个 owner；两个 owner 等价于没有 owner。

**有界类**

- **P6 每条路径必须可达 Closure**：不存在"跑完但不知道算不算完"的节点。
- **P7 每个 Loop 必须有出口**：`maxAttempts` + pass/retry/capExhausted 三出口缺一不可；无出口的 Loop 视同无界。
- **P8 回边只允许由 Loop 引入**：Loop 之外不得存在任何回边。
- **P9 无不可达节点**：声明了却不在任何路径上的节点是设计缺陷，必须报错而非忽略。

**无人值守类**

- **P10 每个 Attempt 必须有 deadline**：超时产出显式事件并走 retry/cap 出口；卡死的节点不得永久挂住 Loop。
- **P11 卡死必须可检测**：节点"存活但未推进"（idle 且 attempt 未收口）是一个可观测事实，不是只能靠人发现的现象。
- **P12 预算必须是运行时强制**：wall-clock / 轮次 / 规模上限由运行时拒绝，而不是欢迎语里的一句纪律。
- **P13 上限超限必须报错，不得静默截断**（沿用外部调研结论：Claude Code 原文 "A silent cap would drop part of the workload without telling the script"）。
- **P14 人工门必须可退化**：`humanMode: "autonomous"` 的必需 Gate 必须携带预批 fallback（沿用现有实现约束）；无人应答时按 `timeoutPolicy` 明确收口，不得悬挂。
- **P15 唤醒必须可观测**：driver 的唤醒依据必须是 durable 事实，不能是 best-effort 的通知（投递失败只 `console.warn` 不算保证）。

### A.4 无人值守为验收不变量

一次编排在**批准之后、无人介入**的前提下必须能跑到明确终态（completed / failed / abandoned 之一，且带不可变证据）。任何需要人盯着才能推进的编排，视为未达标。

### A.5 一等路径与 A2A 的地位

- **编排（第 1–3 步 + 运行时）是一等产品路径。**
- **A2A 是编排使用的传输机制**，同时保留「临时协作」的便捷入口；但它**不再是与编排并列的第二条一等产品叙事**。

### A.6 实施顺序

1. **混合节点后端**（落地 P3）：节点可由 Session 或 sub-agent 承载；分界规则机器化。
2. **无人值守运行时**（落地 P10–P15）：等待原语、Attempt deadline、卡死检测、预算强制、健康投影、唤醒可观测。
3. **原则成文与拆分约束**（落地 P1–P9 + 第 2/3 步的产品面）：原则写进 driver 可教文本；机器可校验子集进 `validateTopology`；draft 必须携带机器可校验的图计划与自然语言解释。

> 未在代码中落地的原则，不得在本文件或 README 中被表述为现状。
